        ...
        <div className="p-4 flex flex-col h-full">
          <div className="h-6 mb-4 ">
            {item.condition && (
              ...
              >
                <span
                  className={`absolute top-0 right-0 inline-block w-2 h-2 ${getConditionLightColor(
                    item.condition
                  )} rounded`}
                >
                  <span
                    className={`absolute top-0 right-0 w-3 h-3 rotate-45 translate-x-1/2 -translate-y-1/2 ${getConditionColor(
                      item.condition
                    )}`}
                  ></span>
                </span>
                <span
                  className={`absolute bottom-0 left-0 inline-block w-2 h-2 rotate-180 ${getConditionLightColor(
                    item.condition
                  )} rounded`}
                >
                  <span
          ...
          <h6 className=" text-base lg:text-sm  font-bold line-clamp-2 text-center h-10 overflow-x-clip">
            {item.name}
          </h6>
          ...
          >
            {Icon &&
              (React.isValidElement(Icon) ? (
                ...
                <Icon className="w-4 h-4 fill-white z-10 transition-transform duration-500 ease-in-out group-hover:translate-x-[4px]" />
              ))}
            {buttonText}
          </button>
        </div>
